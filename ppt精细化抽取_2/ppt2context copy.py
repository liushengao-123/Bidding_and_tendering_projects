...
def extract_structured_text_from_pptx(file_path, start_page=None, end_page=None):
    """
    V-Final-Corrected-3: 采用精确的“拆解-排序-重组”策略，解决所有内容关联问题。
    """
    try:
                ...
                for shape in content_shapes:
                    if shape.shape_type == MSO_SHAPE_TYPE.TABLE:
                        ...
                        for i, para in enumerate(text_frame.paragraphs):
                            ...
                            estimated_top = shape.top + (i / para_count) * shape_height if para_count > 0 else shape.top

                            if line.startswith('■'):
                                content_units.append({'top': estimated_top, 'type': 'key_line', 'text': line})
                            else:
                                content_units.append({'top': estimated_top, 'type': 'text_block', 'text': line})
